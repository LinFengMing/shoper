...
class Console extends CI_Controller
{
    ...
    // 更新管理員
    public function manager_update($id)
    {
        // 頁面資訊
        $view_data = array(
            'title' => "Shoper - Console Manager",
        ...
        if ($this->input->post('rule') == 'update') {
            ...
            if ($dataArray['email'] != '' && $dataArray['nickname'] != '') {
                if (!$this->mod_manager->chk_repeat_email($id, $dataArray['email'])) {
                    if (($this->input->post('password') != '' && $this->input->post('password') === $this->input->post('confirmPassword')) || $this->input->post('password') == '') {
    ...
    public function category_insert()
    {
        ...
        if ($this->input->post('rule') == 'insert') {
            ...
            if ($dataArray['type'] != '') {
                if (!$this->mod_category->get_once_by_type($dataArray['type'])) {
                    $dataArray['id'] = uniqid();
    ...
    public function category_update($id)
    {
        ...
        if ($this->input->post('rule') == 'update') {
            ...
            if ($dataArray['type'] != '') {
                if (!$this->mod_category->chk_repeat_type($id, $dataArray['type'])) {
    ...
    public function product_update($id)
    {
        ...
        if ($this->input->post('rule') == 'update') {
            ...
            if ($_FILES['main_photo_file']['type'] == 'image/png' || $_FILES['main_photo_file']['type'] == 'image/jpeg' || $_FILES['main_photo_file']['type'] == 'image/jpg') {
            ...
            if ($dataArray['title'] != '') {
                if (!$this->mod_product->chk_repeat_title($id, $dataArray['title'])) {
